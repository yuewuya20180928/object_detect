...
def main():
    ...
    if not args.unfreeze_backbone:
        ...
        for layer in backbone.layers:
            if "backbone" in layer.name.lower():
                layer.trainable = False
                # 解冻最后 n 层（但跳过 BatchNorm）
                for sub_layer in layer.layers[-n:]:
                    if "bn" not in sub_layer.name.lower() and "batch" not in sub_layer.name.lower():
                        sub_layer.trainable = True
                break
        logger.info(f"Backbone 解冻最后 {n} 层（跳过 BN）")

    # 学习率调度
    # 实际训练集样本数（从 record 条目数获取，避免硬编码偏差）
    # 117266 是 convert_to_tfrecord.py 日志中确认的训练集有效条数
    _actual_train_samples = 117266
    steps_per_epoch = _actual_train_samples // args.batch_size  # 与实际 batch_size 一致
